...
public:
    ...
    /// While `vendor` and `model` strings are both required, all other values are optional. Passing `std::nullopt`
    /// keeps the current existing value, i.e. does not touch the current value at all. Passing `nullptr`
    /// clear any existing value, other strings replace the corresponding current value.
    void update_chargepoint_information(const std::string& vendor, const std::string& model,
                                        ...
                                        const NullOrOptionalString& iccid = std::nullopt,
                                        const NullOrOptionalString& imsi = std::nullopt,
                                        const NullOrOptionalString& meter_serialnumber = std::nullopt,
                                        const NullOrOptionalString& meter_type = std::nullopt);
